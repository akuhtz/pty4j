...
public class WinPty {
    ...
    public WinPty(String cmdline, String cwd, String env, boolean consoleMode) throws PtyException, IOException {
        ...
        try {
            ...
            spawnCfg =
                INSTANCE.winpty_spawn_config_new(
                    WinPtyLib.WINPTY_SPAWN_FLAG_AUTO_SHUTDOWN | WinPtyLib.WINPTY_SPAWN_FLAG_EXIT_AFTER_SHUTDOWN, null,
                    toWString(cmdline), toWString(cwd), toWString(env), null);
            if (spawnCfg == null) {
    ...
    interface Kern32 extends Library {
        boolean PeekNamedPipe(
            WinNT.HANDLE hFile, Pointer lpBuffer, int nBufferSize, IntByReference lpBytesRead,
        ...
        WinNT.HANDLE CreateNamedPipeA(
            String lpName, int dwOpenMode, int dwPipeMode, int nMaxInstances, int nOutBufferSize, int nInBufferSize,
            int nDefaultTimeout, WinBase.SECURITY_ATTRIBUTES securityAttributes);

        boolean ConnectNamedPipe(WinNT.HANDLE hNamedPipe, WinBase.OVERLAPPED overlapped);

        boolean CloseHandle(WinNT.HANDLE hObject);

        WinNT.HANDLE CreateEventA(
            WinBase.SECURITY_ATTRIBUTES lpEventAttributes, boolean bManualReset, boolean bInitialState, String lpName);
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(WinPty.class);

    // public static WinPtyLib INSTANCE = (WinPtyLib) Native.loadLibrary(getLibraryPath(), WinPtyLib.class);
    public static WinPtyLib INSTANCE;
    static {
        try {
            INSTANCE = (WinPtyLib) Native.loadLibrary(getLibraryPath() /*"winpty"*/, WinPtyLib.class);
        }
        catch (Error e) {
            LOGGER.error("Load WinPtyLib failed.", e);
    ...
    private static String getLibraryPath() {
        try {
            ...
            return PtyUtil.resolveNativeLibrary().getName();
        }
        catch (Exception e) {
    ...
    interface WinPtyLib extends Library {
        ...
        Pointer winpty_spawn_config_new(
            long flags, WString appname, WString cmdline, WString cwd, WString env, PointerByReference err);

        void winpty_spawn_config_free(Pointer cfg);

        boolean winpty_spawn(
            Pointer winpty, Pointer cfg, WinNT.HANDLEByReference process_handle, WinNT.HANDLEByReference thread_handle,
            IntByReference create_process_error, PointerByReference err);
