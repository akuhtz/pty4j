...
/**
 * Provides access to the pseudoterminal functionality on POSIX(-like) systems, emulating such system calls on non POSIX
 * systems.
 */
public class PtyHelpers {
    ...
    public static interface OSFacade {
         ...
         * @return 0 upon success, -1 upon failure (see {@link PtyHelpers#errno()} for details).
         */
        int execve(String command, String[] argv, String[] env);

        /**
         * Returns the window size information for the process with the given FD and stores the results in the given
         * {@link com.pty4j.WinSize} structure.
         *
         * @param fd
         ...
         * @param sig
         *            the signal number to send, for example, 9 to terminate the process.
         * @return a value of <tt>0</tt> upon success, or a non-zero value in case of an error (see
         *         {@link PtyHelpers#errno()} for details).
         */
    ...
    public static Termios createTermios() {
        ...
        boolean isUTF8 = true;
        term.c_iflag =
            JTermios.ICRNL | JTermios.IXON | JTermios.IXANY | IMAXBEL | JTermios.BRKINT | (isUTF8 ? IUTF8 : 0);
        term.c_oflag = JTermios.OPOST | ONLCR;
        term.c_cflag = JTermios.CREAD | JTermios.CS8 | HUPCL;
        term.c_lflag =
            JTermios.ICANON | JTermios.ISIG | JTermios.IEXTEN | JTermios.ECHO | JTermios.ECHOE | ECHOK | ECHOKE
                | ECHOCTL;
     ...
     * @return <code>true</code> if the process with the given process ID is alive, <code>false</code> if it is
     *         terminated.
     */
     ...
     * @param signal
     *            the signal number to send, for example, 9 to terminate the process.
     * @return a value of <tt>0</tt> upon success, or a non-zero value in case of an error.
     */
     ...
     * @param stat
     *            an array of 1 integer in which the status of the process is stored;
     * @param options
    ...
    /**
     * Not public as this method <em>replaces</em> the current process and therefore should be used with caution.
     *
     * @param command
     *            the command to execute.
     */
    ...
    /**
     * Processes the given command + arguments and crafts a valid array of arguments as expected by
     * {@link #execve(String, String[], String[])}.
     *
     * @param command
    ...
    public static int execPty(
        String full_path, String[] argv, String[] envp, String dirpath, String pts_name, int fdm, String err_pts_name,
        int err_fdm, boolean console) {
