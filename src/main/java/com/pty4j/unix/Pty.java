...
public class Pty {
    ...
    private static boolean isOSXLessThanOrEqualTo106() {
        if (System.getProperty("os.name").toLowerCase(Locale.US).startsWith("mac")) {
            ...
            if (strings.length > 1 && strings[0].equals("10") && Integer.valueOf(strings[1]) <= 6)
                return true;
        }
        return false;
    }
     ...
     * </p>
     * <p>
     * <strong>Note:</strong> This method may not be supported on all platforms. Known platforms which support this
     * method are: {@code linux-x86}, {@code linux-x86_64}, {@code solaris-sparc}, {@code macosx}.
     * </p>
     *
     * @param winSize
     *            new size structure.
        ...
        try {
            ...
            if (res != 0) {
                throw new IllegalStateException(
                    "Can set new window size. ioctl returns " + res + ", errorno=" + JTermios.errno());
            }
        }
     ...
     *
     * @return a {@link com.pty4j.WinSize} instance with information about the master side of the Pty, never
     *         <code>null</code>.
     * @throws IOException
    ...
    private static boolean poll(int pipeFd, int fd) {
        ...
        Pollfd pollFd_pipe = new Pollfd();
        pollFd_pipe.fd = pipeFd;
        pollFd_pipe.events = JTermios.POLLIN;
        pollFd_pipe.revents = JTermios.POLLIN;

        Pollfd pollFd_fd = new Pollfd();
        pollFd_fd.fd = fd;
        pollFd_fd.events = JTermios.POLLIN;
        pollFd_fd.revents = JTermios.POLLIN;

        Pollfd[] poll_fds = new Pollfd[] { pollFd_pipe, pollFd_fd };

        while (true) {
            if (JTermios.poll(poll_fds, 2, -1) > 0)
                break;
        ...
        return ((poll_fds[1].fd >> 16) & JTermios.POLLIN) != 0;
    }
